...
indx <- sapply(t_data, is.factor)
t_data[indx] <- lapply(t_data[indx], function(x) as.numeric(as.character(x)))

# Before PCA, we need a column which shows the class of the rows i.e., whether they are a sample or QC. This is to
# help see in the PCA plots how the samples and QCs are distributed. 

Class <- read.csv("Class.csv", header=T)
t_data <- cbind(Class, t_data)
...
data_pareto <- paretoscale(t_data[,c(2:2629)], exclude=F)

### PCA AFTER PARETO ###

data_pareto.pca <- prcomp(data_pareto, center=TRUE, scale. = TRUE)

autoplot(data_pareto.pca, data=t_data, colour="Class" )+ ggtitle("PCA Plot of QCs and Samples after Pareto Scaling")
...
library(KODAMA)
data_norm <- normalization(data_pareto,Xtest=NULL, method = "pqn",ref=NULL)
data_norm <- as.data.frame(data_norm["newXtrain"])
