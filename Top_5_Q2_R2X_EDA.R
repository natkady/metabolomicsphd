...
top_5_q2_r2x <- arrange(id1_vs_id2,R2X.plsda.out,Q2Y.plsda.out)
head(top_5_q2_r2x)

# Create a separate 'data frame' of the 5 top combination iterations
iter <- top_5_q2_r2x[1:5,c(1,2)]
...
#save xscore plots 
setwd("I:/Documents/PhD files/Nik/Nik Lockdown/Plots/top5_q2y_r2x")
for (i in 1:nrow(iter)) {
...
#PLSDA - 3rd, 4th and 5th combinations have different infant sizes
plsda_list <- list()
for(i in 3:5){ 
  # want to do placenta_id vs placenta_id 
  temp_dat <- batch1[batch1$Placenta_ID%in%iter[i,],]
...
#PLSDA -2nd, 3rd and 5th iter combinations have different infant sexes
plsda_list <- list()
for(i in c(2,3,5)){ 
  # want to do placenta_id vs placenta_id 
  temp_dat <- batch1[batch1$Placenta_ID%in%iter[i,],]
