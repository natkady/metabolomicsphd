...
# with regards to Q2Y and R2Y
top_5_q2_r2y <- arrange(id1_vs_id2,R2Y.plsda.out,Q2Y.plsda.out)
head(top_5_q2_r2y)

# Create a separate 'data frame' of the 5 top combination iterations
iter <- top_5_q2_r2y[1:5,c(1,2)]
...
#save xscore plots 
setwd("I:/Documents/PhD files/Nik/Nik Lockdown/Plots/top5_q2y_r2y")
for (i in 1:nrow(iter)) {
...
# for PLSDA, only the 5th combination of the top 5 had different infant sizes,
...
#PLSDA - only 1st and 4th  iter combination have different sexes. 
plsda_list <- list()
for(i in c(1,4)){ 
  # want to do placenta_id vs placenta_id 
  temp_dat <- batch1[batch1$Placenta_ID%in%iter[i,],]
